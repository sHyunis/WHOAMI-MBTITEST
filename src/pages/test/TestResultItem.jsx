...
import useBearsStore from "../../zustand/bearsStore"; // Zustand store import
...
const TestResultItem = ({ result, onUpdate, onDelete }) => {
  ...
  return (
    <div className="w-[350px] mx-auto p-6 bg-gray-800 rounded-lg shadow-lg text-white mb-4">
      <div className="flex justify-between items-center bg-gray-800 pb-3 mb-3">
        <h4 className="text-xl text-white bg-gray-800 font-semibold">
          {result.nickname}
        </h4>
        <p className="text-sm text-white bg-gray-800">{formattedDate}</p>
      </div>
      <p className="text-2xl font-bold text-yellow-400 mb-4 bg-gray-800">
        {result.result}
      </p>
      <p className="text-base text-white mb-4 bg-gray-800">{description}</p>
      {isOwner && (
        <div className="flex justify-end space-x-4 bg-gray-800 ">
          <button
